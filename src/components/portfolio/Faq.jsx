      ...
      <div className="max-w-3xl mx-auto space-y-4">
        {faqData.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`rounded-2xl border border-white/10 bg-white/[0.01] backdrop-blur-md overflow-hidden transition-all duration-300 hover:border-cyan-glow/30 ${
                isOpen ? "border-cyan-glow/50 bg-white/[0.03] shadow-[0_0_20px_rgba(6,182,212,0.08)]" : ""
              }`}
            >
              <button
                onClick={() => toggleIndex(index)}
                className="w-full flex items-center justify-between p-5 text-left font-mono-ui text-sm sm:text-base font-medium text-white transition-colors hover:text-cyan-400 cursor-pointer"
              >
                <div className="flex items-center gap-3">
                  <HelpCircle className="size-4 shrink-0 text-cyan-400/80" />
                  <span>{item.q}</span>
                </div>
                {isOpen ? (
                  <ChevronUp className="size-4 text-cyan-400 shrink-0 ml-3" />
                ) : (
